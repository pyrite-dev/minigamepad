    ...
    #ifndef RSGL_GL_NO_LEGACY
    {
        ...
        for (i = 0; i < info->len; i++) {
            ...
            for (j = info->batches[i].start; j < info->batches[i].len; j++) { 
                RSGL_point3D point =  RSGL_POINT3D(info->verts[vIndex], info->verts[vIndex + 1],  info->verts[vIndex + 2]);
                if (RSGL_args.program == 0) {
                    point = (RSGL_point3D){RSGL_GET_WORLD_POINT(point.x, point.y, point.z)};
                }
