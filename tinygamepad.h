...
/*
	(MAKE SURE TG_IMPLEMENTATION is in exactly one header or you use -D TG_IMPLEMENTATION)
	#define TG_IMPLEMENTATION - makes it so source code is included with header
*/

/*
	#define TG_IMPLEMENTATION - (required) makes it so the source code is included
	#define TG_ALLOC x  - choose the default allocation function (defaults to standard malloc)
	#define TG_FREE x  - choose the default deallocation function (defaults to standard free)

	#define TG_EXPORT - use when building tinygamepad 
	#define TG_IMPORT - use when linking with tinygamepad (not as a single-header)

	#define TG_USE_INT - force the use c-types rather than stdint.h (for systems that might not have stdint.h (msvc))
	#define tg_bool x - choose what type to use for bool, by default u32 is used

    #define TG_MAX_GAMEPADS x - set the max number of gamepads (4 by default)
*/
...
#if !defined(TG_MEMCPY) || !defined(TG_STRNCMP) || !defined(TG_STRNCPY) || !defined(TG_MEMSET) || !defined(TG_STRCSPN) || !defined(TG_STRSPN)
	#include <string.h>
#endif

#ifndef TG_MEMSET
	#define TG_MEMSET(ptr, value, num) memset(ptr, value, num)
#endif

#ifndef TG_MEMCPY
	#define TG_MEMCPY(dist, src, len) memcpy(dist, src, len)
#endif

#ifndef TG_STRNCMP
	#define TG_STRNCMP(s1, s2, max) strncmp(s1, s2, max)
#endif

#ifndef TG_STRNCPY
	#define TG_STRNCPY(dist, src, len) strncpy(dist, src, len)
#endif

#ifndef TG_STRCSPN 
    #define TG_STRCSPN(str1, str2) strcspn(str1, str2) 
#endif

#ifndef TG_STRSPN
    #define TG_STRSPN(str1, str2) strspn(str1, str2)
#endif 

#if !defined(TG_SNPRINTF) || !defined(TG_FPRINTF) 
    /* required when using TG_DEBUG */
    #include <stdio.h>
#endif


#if !defined(TG_SNPRINTF) 
    #define TG_SNPRINTF snprintf
#endif

#if !defined(TG_FPRINTF) 
    #define TG_FPRINTF fprintf
#endif

#if defined(TG_EXPORT) ||  defined(TG_IMPORT)
	#if defined(_WIN32)
		#if defined(__TINYC__) && (defined(TG_EXPORT) ||  defined(TG_IMPORT))
			#define __declspec(x) __attribute__((x))
		#endif

		#if defined(TG_EXPORT)
...
#ifndef TG_ENUM
	#define TG_ENUM(type, name) type name; enum
#endif
...
#ifndef TG_BOOL_DEFINED
    #define TG_BOOL_DEFINED
    typedef u8 tg_bool;
#endif

#ifndef TG_MAX_GAMEPADS
    #define TG_MAX_GAMEPADS 4
#endif

#define TG_BOOL(x) (tg_bool)((x) ? TG_TRUE : TG_FALSE) /* force an value to be 0 or 1 */
#define TG_TRUE (tg_bool)1
#define TG_FALSE (tg_bool)0

typedef TG_ENUM(i8, tg_button) {
    TG_BUTTON_UNKNOWN = -1,
    TG_BUTTON_SOUTH,           /**< Bottom face button (e.g. Xbox A button) */
    TG_BUTTON_EAST,            /**< Right face button (e.g. Xbox B button) */
    TG_BUTTON_WEST,            /**< Left face button (e.g. Xbox X button) */
    TG_BUTTON_NORTH,           /**< Top face button (e.g. Xbox Y button) */
    TG_BUTTON_BACK,
    TG_BUTTON_GUIDE,
    TG_BUTTON_START,
    TG_BUTTON_LEFT_STICK,
    TG_BUTTON_RIGHT_STICK,
    TG_BUTTON_LEFT_SHOULDER,
    TG_BUTTON_RIGHT_SHOULDER,
    TG_BUTTON_DPAD_UP,
    TG_BUTTON_DPAD_DOWN,
    TG_BUTTON_DPAD_LEFT,
    TG_BUTTON_DPAD_RIGHT,
    TG_BUTTON_LEFT_TRIGGER,
    TG_BUTTON_RIGHT_TRIGGER,
 /* extras */
    TG_BUTTON_MISC1,           /**< Additional button (e.g. Xbox Series X share button, PS5 microphone button, Nintendo Switch Pro capture button, Amazon Luna microphone button, Google Stadia capture button) */
    TG_BUTTON_RIGHT_PADDLE1,   /**< Upper or primary paddle, under your right hand (e.g. Xbox Elite paddle P1) */
    TG_BUTTON_LEFT_PADDLE1,    /**< Upper or primary paddle, under your left hand (e.g. Xbox Elite paddle P3) */
    TG_BUTTON_RIGHT_PADDLE2,   /**< Lower or secondary paddle, under your right hand (e.g. Xbox Elite paddle P2) */
    TG_BUTTON_LEFT_PADDLE2,    /**< Lower or secondary paddle, under your left hand (e.g. Xbox Elite paddle P4) */
    TG_BUTTON_TOUCHPAD,        /**< PS4/PS5 touchpad button */
    TG_BUTTON_MISC2,           /**< Additional button */
    TG_BUTTON_MISC3,           /**< Additional button */
    TG_BUTTON_MISC4,           /**< Additional button */
    TG_BUTTON_MISC5,           /**< Additional button */
    TG_BUTTON_MISC6,           /**< Additional button */
    TG_BUTTON_COUNT
};

typedef TG_ENUM(i8, tg_axis) {
    TG_AXIS_UNKNOWN = -1,
    TG_AXIS_LEFT_X,
    TG_AXIS_LEFT_Y,
    TG_AXIS_RIGHT_X,
    TG_AXIS_RIGHT_Y,
    TG_AXIS_LEFT_TRIGGER,
    TG_AXIS_RIGHT_TRIGGER,
    TG_AXIS_HAT_DPAD_LEFT_RIGHT,
    TG_AXIS_HAT_DPAD_LEFT = TG_AXIS_HAT_DPAD_LEFT_RIGHT,
    TG_AXIS_HAT_DPAD_RIGHT = TG_AXIS_HAT_DPAD_LEFT_RIGHT,
    TG_AXIS_HAT_DPAD_UP_DOWN,
    TG_AXIS_HAT_DPAD_UP = TG_AXIS_HAT_DPAD_UP_DOWN,
    TG_AXIS_HAT_DPAD_DOWN = TG_AXIS_HAT_DPAD_UP_DOWN,
/* extras */
    TG_AXIS_THROTTLE,
    TG_AXIS_RUDDER,
    TG_AXIS_WHEEL,
...
#elif defined(TG_WASM)
typedef struct tg_gamepad_src {     

} tg_gamepad_src;
#endif

typedef struct tg_button_state {
    tg_bool supported;
    tg_bool current;
    tg_bool prev;
} tg_button_state;
...
typedef struct tg_gamepad {
    char name[128];
    char guid[33];

    tg_button_state buttons[TG_BUTTON_COUNT];
...
typedef struct tg_gamepads {
    tg_gamepad gamepads[TG_MAX_GAMEPADS];
    
    tg_gamepad_list list;
    tg_gamepad_list free_list;
...
TGDEF void tg_gamepads_init(tg_gamepads* gamepads);
TGDEF tg_bool tg_gamepads_update(tg_gamepads* gamepads, tg_event* event);
TGDEF void tg_gamepads_free(tg_gamepads* gamepads);
...
TGDEF void tg_gamepads_free_platform(tg_gamepads* gamepads);
TGDEF tg_bool tg_gamepad_update_platform(tg_gamepad* gamepad, tg_event* event);
TGDEF void tg_gamepad_release_platform(tg_gamepad* gamepad);
...
TGDEF void tg_mappings_init(void);
/* public/global API implementation */

void tg_gamepads_init(tg_gamepads* gamepads) {
    ...
    tg_gamepads_init_platform(gamepads);
}


tg_bool tg_gamepads_update(tg_gamepads* gamepads, tg_event* event) {
    tg_gamepad* cur;

    if (tg_gamepads_update_platform(gamepads, event)) 
        return TG_TRUE;
    for (cur = gamepads->list.head; cur != NULL; cur = cur->next) {
        if (tg_gamepad_update_platform(cur, event)) return TG_TRUE;
    }
...
void tg_list_swap_gamepad(tg_gamepad_list* from, tg_gamepad_list* to, tg_gamepad* gamepad) {
    ...
    TG_MEMSET(gamepad, 0, sizeof(tg_gamepad));
...
tg_gamepad* tg_gamepad_find(tg_gamepads* gamepads) {
    tg_gamepad* gamepad = gamepads->free_list.cur; 
    if (gamepad == NULL)
        return NULL;
...
void tg_gamepad_release(tg_gamepads* gamepads, tg_gamepad* gamepad) {
    tg_gamepad_release_platform(gamepad);
    tg_list_swap_gamepad(&gamepads->list, &gamepads->free_list, gamepad);
}
...
tg_gamepad* tg_linux_setup_gamepad(tg_gamepads* gamepads, const char* full_path) {
    ...
    TG_STRNCPY(gamepad->src.full_path, full_path, 256);
    ...
    if ((axisCount == 0 && buttonCount == 0) || buttonCount > TG_BUTTON_COUNT + 10) {
        tg_gamepad_release(gamepads, gamepad);
        return NULL;
    }
    ...
    if (ioctl(gamepad->src.fd, EVIOCGNAME(sizeof(gamepad->name)), gamepad->name) < 0)
        TG_STRNCPY(gamepad->name, "Unknown", sizeof(gamepad->name));

    if (id.vendor && id.product && id.version) {
        TG_SNPRINTF(gamepad->guid, sizeof(gamepad->guid), "%02x%02x0000%02x%02x0000%02x%02x0000%02x%02x0000",
        ...
        TG_SNPRINTF(gamepad->guid, sizeof(gamepad->guid), "%02x%02x0000%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x00",
                 id.bustype & 0xff, id.bustype >> 8,
    ...
    for (unsigned int btn = BTN_MISC; btn < KEY_CNT; btn++) {
        ...
        if (!isBitSet(btn, keyBits)) {
            gamepad->buttons[key].supported = TG_FALSE;
            continue;
        }
       ...
       if (!isBitSet(axis, absBits)) {
        ...
        switch (axis) {
            case ABS_HAT0X:
                ...
                gamepad->buttons[TG_BUTTON_DPAD_LEFT].current = 0; 
                gamepad->buttons[TG_BUTTON_DPAD_RIGHT].supported = TG_TRUE;
                gamepad->buttons[TG_BUTTON_DPAD_RIGHT].current = 0; 
                deadzone = 0;
                break;
            case ABS_HAT0Y:
                gamepad->buttons[TG_BUTTON_DPAD_UP].supported = TG_TRUE;
                gamepad->buttons[TG_BUTTON_DPAD_UP].current = 0; 
                gamepad->buttons[TG_BUTTON_DPAD_DOWN].supported = TG_TRUE;
                gamepad->buttons[TG_BUTTON_DPAD_DOWN].current = 0; 
                deadzone = 0;
                break;
            case ABS_HAT1X:
            case ABS_HAT1Y:
            ...
            case ABS_HAT3Y: 
                ...
                break;
            case ABS_Z:
                gamepad->buttons[TG_BUTTON_LEFT_TRIGGER].supported = TG_TRUE;
                gamepad->buttons[TG_BUTTON_LEFT_TRIGGER].current = 0;
                deadzone = 0;
                break;
            case ABS_RZ:
                gamepad->buttons[TG_BUTTON_RIGHT_TRIGGER].supported = TG_TRUE;
                gamepad->buttons[TG_BUTTON_RIGHT_TRIGGER].current = 0;
                deadzone = 0;
                break;
            default:
                deadzone = 15;
...
void tg_gamepads_init_platform(tg_gamepads* gamepads) {
    ...
    while ((dp = readdir(dfd)) != NULL) {
        ...
        TG_SNPRINTF(full_path, sizeof(full_path), "/dev/input/%s", dp->d_name); 
        tg_linux_setup_gamepad(gamepads, (const char*)full_path);
    }
...
tg_bool tg_gamepads_update_platform(tg_gamepads* gamepads, tg_event* event) {
    ...
    while (size > offset) {
        ...
        TG_SNPRINTF(full_path, sizeof(full_path), "%s%s", path, e->name);
        full_path[255] = '\0';

        if (e->mask & (IN_CREATE | IN_ATTRIB)) { 
            tg_gamepad* gamepad = tg_linux_setup_gamepad(gamepads, full_path);
            if (gamepad) {
                if (event != NULL) {
                    event->type = TG_EVENT_GAMEPAD_CONNECT;
...
#define emulate_button(button_code, axis, min, max, flip) { \
    if ((gamepad->axes[axis].value >= max && flip!gamepad->buttons[button_code].current) || \
            (gamepad->axes[axis].value <= min && flip gamepad->buttons[button_code].current)) { \
            gamepad->buttons[button_code].prev =  gamepad->buttons[button_code].current; \
            gamepad->buttons[button_code].current = flip(gamepad->axes[axis].value >= max) ? 1 : 0; \
            if (event != NULL) { \
                event->gamepad = gamepad; \
                event->button = button_code; \
                event->type = flip(gamepad->axes[button_code].value >= max) ? TG_EVENT_BUTTON_PRESS : TG_EVENT_BUTTON_RELEASE; \
                return 1; \
            } \
        } \
}
...
tg_bool tg_gamepad_update_platform(tg_gamepad* gamepad, tg_event* event) {
    ...
    emulate_button(TG_BUTTON_RIGHT_TRIGGER, TG_AXIS_RIGHT_TRIGGER, -98, 98, )

    emulate_button(TG_BUTTON_DPAD_LEFT, TG_AXIS_HAT_DPAD_LEFT, -100, 0, !)
    emulate_button(TG_BUTTON_DPAD_RIGHT, TG_AXIS_HAT_DPAD_RIGHT, 0, 100, )
    ...
    switch (ev.type) {
        case EV_KEY: {
            tg_button btn = tg_get_gamepad_button(gamepad, (u8)(gamepad->src.keyMap[ev.code - BTN_MISC])); 
            if (btn == TG_BUTTON_UNKNOWN) {
                btn = tg_get_gamepad_button_platform(ev.code); 
            }
            ...
            if (event != NULL) {
                event->gamepad = gamepad;
                event->button = btn;
                event->type = ev.value ? TG_EVENT_BUTTON_PRESS  : TG_EVENT_BUTTON_RELEASE; 
            }
            return TG_TRUE;
        }
        case EV_ABS: {
            ...
            if (gamepad->axes[axis].supported) {
                int deadzone = gamepad->axes[axis].deadzone;
                int16_t event_val = (int16_t)(normalized * 100);
                if (abs(event_val) < deadzone) {
                    event_val = 0;
                }

                gamepad->axes[axis].value = event_val;
            }
...
void tg_gamepad_release_platform(tg_gamepad* gamepads-) {
...
typedef struct tg_mapping {
    char            name[128];
    char            guid[33];
    tg_element buttons[16];
    tg_element axes[6];

    tg_button rButtons[256];
    tg_axis rAxes[TG_AXIS_COUNT];
} tg_mapping;

#define TG_JOYSTICK_AXIS     1 
#define TG_JOYSTICK_BUTTON   2
#define TG_JOYSTICK_HATBIT   3

typedef struct mappings_data {
    tg_mapping       mappings[1300];
    int               mappingCount;
    int               mappingMax;
} mappings_data;
...
tg_button tg_get_gamepad_button(tg_gamepad* gamepad, u8 btn) {
...
tg_axis tg_get_gamepad_axis(tg_gamepad* gamepad, u8 axis) {
    if (gamepad->mapping == NULL) {
        return TG_AXIS_UNKNOWN;
    }
    
    if (axis >= TG_AXIS_COUNT) { 
        return TG_AXIS_UNKNOWN;
    }
...
static tg_mapping* findMapping(const char* guid) {
    int i; 
...
static tg_mapping* findMappingPermisive(const char* guid) {
    int i; 
    for (i = 0;  i < mappings.mappingCount;  i++) {
...
tg_mapping* tg_gamepad_find_valid_mapping(tg_gamepad* js) {
    tg_mapping* mapping = findMapping(js->guid);
    if (mapping == NULL) {      
...
static tg_bool parseMapping(tg_mapping* mapping, const char* string) {
    ...
    while (substr[0]) {
        ...
        substr += TG_STRCSPN(substr, ",");
        substr += TG_STRSPN(substr, ",");
    }
...
tg_bool tg_update_gamepad_mappings(tg_gamepads* gamepads, const char* string) {
    ...
    while (*substr) {
        ...
        if (length < sizeof(line)) {
            ...
            if (parseMapping(&mapping, line)) {
                tg_mapping* previous = findMapping(mapping.guid);
                if (previous)
                    *previous = mapping;
                else {
    ...
    for (cur = gamepads->list.head; cur; cur = cur->next) {
        cur->mapping = tg_gamepad_find_valid_mapping(cur);
    }
