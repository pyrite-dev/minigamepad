...
void _libevdev_log_msg(const struct libevdev *dev,
  ...
  if (dev && dev->log.device_handler)
    dev->log.device_handler(dev, priority, dev->log.userdata, file, line, func,
                            format, args);
  else
...
#define AXISBIT(_slot, _axis) (_slot * ABS_MT_CNT + _axis - ABS_MT_MIN)
  ...
  for (axis = ABS_MT_MIN; axis <= ABS_MT_MAX; axis++) {
    ...
    if (rc < 0) {
      ...
      for (slot = 0; slot < dev->num_slots; slot++) {
        ...
        if (axis == ABS_MT_TRACKING_ID && *slot_value(dev, slot, axis) != -1 &&
            mt_state->val[slot] != -1) {
  ...
  for (slot = 0; slot < dev->num_slots; slot++) {
    ...
    for (axis = ABS_MT_MIN; axis <= ABS_MT_MAX; axis++) {
      if (axis == ABS_MT_SLOT || !libevdev_has_event_code(dev, EV_ABS, axis))
        continue;
...
LIBEVDEV_EXPORT int libevdev_next_event(struct libevdev *dev,
  ...
  /* Always read in some more events. Best case this smoothes over a potential
     ...
     Except if the fd is in blocking mode and we still have events from the last
     read, don't read in any more.
   */
  do {
